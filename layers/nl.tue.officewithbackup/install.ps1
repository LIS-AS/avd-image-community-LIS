...
New-NetFirewallRule -DisplayName "Allow Azure File Share" -Direction Outbound -remoteport 445 -Protocol TCP -Action Allow -remoteaddres "20.209.177.70"

# For backup user files
# Check if folder not exists, and create it
if (-not(Test-Path C:\Windows\Updates -PathType Container)) {
    New-Item -path C:\Windows\Updates -ItemType Directory
}
# Set access to backup script for system and admin only
Invoke-Expression -Command:"icacls C:\Windows\Updates /inheritance:r /grant SYSTEM:'(OI)(CI)(F)' Administrators:'(OI)(CI)(F)'"
# Copy backupscript to this folder
if (-not(Test-Path C:\Windows\Updates\SYBackup.ps1 -PathType Container)) {
    Copy-Item -path ".\Backup\SYBackup.ps1" -destination "C:\Windows\Updates\SYBackup.ps1"
}
# Create Scheduled Task by importing XML file
Register-ScheduledTask -TaskName "SYBackup" -Xml (Get-Content ".\Backup\SYBackup.xml" | Out-String) -Force